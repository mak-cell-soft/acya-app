 ...
 * Handles editing of customer delivery notes.
 * Includes manual adjustment of the final TTC price and validation against invoiced status.
 */
@Component({
  selector: 'app-edit-customer-document',
  templateUrl: './edit-customer-document.component.html',
  styleUrl: './edit-customer-document.component.scss'
})
export class EditCustomerDocumentComponent implements OnInit {
  ...
  public isQuantityValid = isQuantityValid;
  ...
  displayedColumns = [
    'index', 'article', 'unit_price_ht', 'quantity', 'cost_ht',
    'discountPercentage', 'cost_net_ht', 'actions'
  ];
    ...
    this.appVarService.GetAll('Tva').subscribe(tvas => {
      ...
      this.articleService.GetAll().subscribe(articles => {
        ...
        this.docService.GetById(docId).subscribe({
          next: (doc: Document) => {
            // Check if document is already invoiced
            if (doc.isinvoiced) {
              this.toastr.warning('Ce document est déjà facturé et ne peut pas être modifié.', 'Action bloquée');
              this.router.navigate(['/home/merchandise/customerdelivery']);
              return;
            }
            ...
            this.documentForm.patchValue({
              customerReference: this.document.supplierReference
            });
            
            // Calculate initial extraDiscount if any
            // doc.total_net_ttc might have been manually adjusted during creation
            // We assume the sum of merchandises cost_ttc is the original NetTTC
            const sumMerchTTC = doc.merchandises?.reduce((sum, item) => sum + item.cost_ttc, 0) || 0;
            this.extraDiscount = doc.total_net_ttc - sumMerchTTC;
  ...
  onOptionSelected(articleId: number) {
    ...
    if (article) {
      ...
      // Default price from article
      this.merchandiseForm.patchValue({
        unit_price_ht: article.sellprice_ht || 0
      });
    }
  }
  ...
  calculateTotals(data: Merchandise[]) {
    ...
    const baseNetTTC = data.reduce((sum, item) => sum + item.cost_ttc, 0);
    ...
    this.netTTC_doc$.next(baseNetTTC + this.extraDiscount);
  }

  onFinalPriceChange(newTotalTtc: number) {
    const baseNetTTC = this.merchandisDocument.data.reduce((sum, item) => sum + item.cost_ttc, 0);
    this.extraDiscount = newTotalTtc - baseNetTTC;
    this.netTTC_doc$.next(newTotalTtc);
  }
  ...
  recalculateFormFields(discountPct: number, quantity: number) {
    ...
    const tvaEntry = this.TVAs.find(t => t.id === (this.selectedArticle.tvaid || this.merchandiseForm.get('tva')?.value));
    const tvaPct = parseFloat(tvaEntry?.value?.replace('%', '') || '0');
  ...
  openAddQuantityModal(article: Article) {
    const dialogRef = this.dialog.open(AddLengthsModalComponent, {
      ...
      data: {
        ...
        isPurchase: false // Customer delivery note is SALE
      }
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res) {
  ...
  onSubmit() {
    if (this.documentForm.valid && this.merchandisDocument.data.length > 0) {
      const updatedDoc: Document = {
        ...this.document,
        supplierReference: this.documentForm.get('customerReference')?.value,
    ...
    } else if (this.merchandisDocument.data.length === 0) {
      this.toastr.warning('Le document doit contenir au moins un article.');
    }
  }
