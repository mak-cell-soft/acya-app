...
export default function ProvidersPage() {
  ...
  const handleSave = async (data: Partial<Supplier>) => {
    if (selectedSupplier) {
      ...
      await createMutation.mutateAsync(data as Supplier);
    }
    setIsFormOpen(false);
  };
        ...
        <tr>
          <td colSpan={6} className="p-24 text-center">
            <div className="flex flex-col items-center gap-4">
              <div className="w-12 h-12 border-4 border-corp-blue-100 border-t-corp-blue-600 rounded-full animate-spin" />
        ...
        <tr>
          <td colSpan={6} className="p-24 text-center">
            <div className="max-w-xs mx-auto space-y-3">
              ...
              <p className="text-sm font-bold text-corp-blue-900">Aucun fournisseur trouvé</p>
    ...
    return filteredSuppliers.map((item) => (
      <React.Fragment key={item.id}>
        ...
        >
          ...
          <td className="p-6">
            <div className="font-mono text-xs font-bold text-corp-blue-600 bg-corp-blue-50 px-3 py-1.5 rounded-lg inline-block border border-corp-blue-100/50">
              {item.taxregistrationnumber || "—"}
            </div>
          </td>
          <td className="p-6 text-center">
            <Badge variant="outline" className="bg-white border-corp-blue-100 text-corp-blue-600 font-bold rounded-xl px-3 py-1">
            ...
            <Badge 
              className={cn(
                ...
                item.type === 'Both' 
                  ...
                  : "bg-corp-blue-100 text-corp-blue-700 border border-corp-blue-200"
              )}
            >
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="rounded-2xl border-corp-blue-100 p-2 w-52 shadow-xl bg-white/95 backdrop-blur-md">
                  ...
                  <DropdownMenuItem 
                    className="gap-3 font-bold text-corp-blue-900 rounded-xl p-3 cursor-pointer hover:bg-corp-blue-50"
                  >
                    <History className="w-4 h-4 text-corp-blue-500" /> Historique Achats
                  </DropdownMenuItem>
                  <div className="h-px bg-corp-blue-50 my-1" />
          ...
          {expandedId === item.id && (
            <tr>
              <td colSpan={6} className="p-0 border-none">
                ...
                >
                  <div className="p-10 grid grid-cols-1 md:grid-cols-4 gap-12 border-b border-corp-blue-50">
                    <div className="space-y-6">
                      ...
                      <div className="space-y-4">
                        <div className="flex items-start gap-4 text-sm text-sand-600 font-bold group">
                          <div className="w-8 h-8 rounded-lg bg-white shadow-sm flex items-center justify-center text-corp-blue-600 group-hover:scale-110 transition-transform">
                    ...
                    <div className="space-y-6 md:border-l md:border-corp-blue-100 md:pl-10">
                      <h4 className="text-[0.65rem] font-black text-sand-400 uppercase tracking-[0.2em]">Responsable</h4>
                      <div className="flex items-center gap-4 group">
                        <div className="w-10 h-10 rounded-full bg-corp-blue-900 flex items-center justify-center text-white text-xs font-bold group-hover:rotate-12 transition-transform">
                          {item.firstname?.[0]}{item.lastname?.[0]}
                        </div>
                        <div>
                          <div className="text-sm font-black text-corp-blue-900">{item.firstname} {item.lastname}</div>
                      ...
                      {item.bankname ? (
                        <div className="space-y-3">
                          <div className="flex items-center gap-2">
                            <Badge variant="outline" className="bg-corp-blue-600 text-white border-transparent px-2 py-0 font-black text-[0.6rem]">
                              {item.bankname}
                            </Badge>
                          </div>
                          <div className="font-mono text-xs text-corp-blue-900 font-bold break-all bg-white p-3 rounded-xl border border-corp-blue-50 shadow-sm">
                    ...
                    <div className="space-y-6 md:border-l md:border-corp-blue-100 md:pl-10 flex flex-col justify-between">
                      <div>
                      ...
                      <Button 
                        className="w-full bg-corp-blue-900 text-white h-11 font-black text-xs uppercase tracking-widest hover:bg-black transition-all shadow-lg shadow-corp-blue-900/10"
    ...
    <DashboardLayout>
      <div className="space-y-8 animate-in fade-in duration-700">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-4xl font-black text-corp-blue-900 tracking-tight">Gestion des Fournisseurs</h1>
        ...
        <Card className="border-corp-blue-100/50 shadow-2xl shadow-corp-blue-900/5 rounded-2xl overflow-hidden bg-white/80 backdrop-blur-md">
          <CardHeader className="border-b border-corp-blue-50 p-8 bg-white/50">
            <div className="flex flex-col md:flex-row md:items-center gap-6">
              <div className="relative flex-1">
                ...
                <Input 
                  ...
                  className="pl-12 h-12 rounded-2xl border-corp-blue-50 bg-sand-50/30 focus:border-corp-blue-600 focus:ring-corp-blue-600 transition-all font-medium"
                  value={searchTerm}
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="h-8 w-px bg-corp-blue-100 mx-2 hidden md:block" />
                <div className="flex items-center gap-3 px-5 py-3 bg-corp-blue-50 rounded-2xl border border-corp-blue-100">
                  <Warehouse className="w-5 h-5 text-corp-blue-600" />
                  <span className="text-sm font-black text-corp-blue-900 tracking-tight">
                    {suppliers.length} <span className="text-corp-blue-400 font-bold ml-1 uppercase text-[0.65rem]">Total</span>
                  </span>
                </div>
              </div>
            </div>
          ...
          <CardContent className="p-0">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-sand-50/30 border-b border-corp-blue-50">
