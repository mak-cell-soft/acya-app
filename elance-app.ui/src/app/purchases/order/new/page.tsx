 ...
 * - Slate-900 dominates with warm Amber accents.
 * - Distinctive typography pairs playfair-serif titles with clean monospace numbers (font-mono / IBM Plex Mono).
 * - Soft glassmorphism cards and smooth staggered micro-animations enhance user interaction.
 * 
...
function NewSupplierOrderPageContent() {
  ...
  // 1.1 Approval settings for threshold detection
  const [approvalThreshold, setApprovalThreshold] = useState<number | null>(null);

  useEffect(() => {
    const entId = user?.enterpriseId ? parseInt(user.enterpriseId) : 0;
    if (entId > 0) {
      approvalService.getConfig(entId)
        .then(config => {
          if (config && config.thresholdAmount !== undefined) {
            setApprovalThreshold(config.thresholdAmount);
          }
        })
        .catch(err => console.error('Failed to load approval threshold configuration:', err));
    }
  }, [user?.enterpriseId]);
  ...
  // 2. Form Entête State variables
  const [selectedSupplier, setSelectedSupplier] = useState<Supplier | null>(null);
  const [supplierSearchQuery, setSupplierSearchQuery] = useState('');
  const [isSupplierDropdownOpen, setIsSupplierDropdownOpen] = useState(false);
  const [supplierReference, setSupplierReference] = useState<string>(''); // Internal ref note
  ...
  const [docStatus, setDocStatus] = useState<DocStatus>(DocStatus.Pending); // Defaults to Pending (9)
  const [notes, setNotes] = useState<string>(''); // General document observations

  // Status mapping for localized UI badges
  const getStatusLabel = (status: DocStatus) => {
    switch (status) {
      case DocStatus.Pending: return 'En attente';
      case DocStatus.Sent: return 'Envoyé';
      case DocStatus.PartiallyDelivered: return 'Partiellement livré';
      case DocStatus.Delivered: return 'Livré';
      default: return 'Créé';
    }
  };

  // 3. New Article Row State (Form fields for adding a row)
  ...
  const [newRowLastPurchasePrice, setNewRowLastPurchasePrice] = useState<number>(0);
  const [isLoadingLastPrice, setIsLoadingLastPrice] = useState<boolean>(false);
  ...
  // Wood lengths dialog overlay modal state
  ...
  // Filters suppliers in header selector
  ...
  // Loading state for submitting the transaction
  const [isSubmitting, setIsSubmitting] = useState(false);

  // Rows list added to the order
  const [rows, setRows] = useState<MerchandRow[]>([]);

  // 5. Watch selected Article in form to pre-populate last purchase price and TVA
  useEffect(() => {
    if (newRowArticle) {
  ...
  // 6. Handle currency and exchange rates automatically
  const handleCurrencyChange = (curr: string) => {
    ...
    if (curr === 'TND') {
      ...
      exchangeRateService.getExchangeRate(targetCurrency, 'TND')
        ...
        .catch(() => {
          setExchangeRate(curr === 'EUR' ? 3.4 : 3.1); // Dynamic standard fallbacks
        })
        .finally(() => setIsSubmitting(false));
    }
  };

  // 7. Calculate calculations for a single grid row reactively
  const calculateRowCalculations = (row: MerchandRow): MerchandRow => {
    ...
    if (row.line_type === LineType.TransportFee) {
      costHT = priceHT; // Transport row represents direct sum cost
      row.quantity = 1;
    }
  ...
  // 8. Add dynamic merchandise row to document grid
  ...
  // 9. Add transport row dynamically to document grid
  const handleAddTransportRow = () => {
    ...
    const newRow: MerchandRow = {
      ...
      isinvoicible: true,
      allownegativstock: false,
      tva_percentage: 19, // C# standard 19% TVA for logistical items
    ...
    toast.success("Ligne logistique de transport insérée.");
  };

  // Remove row from document list
  const handleRemoveRow = (index: number) => {
    setRows(rows.filter((_, i) => i !== index));
    toast.info("Ligne retirée de la liste.");
  };
  ...
  // 10. Document totals computations
  const totals = useMemo(() => {
    ...
    return {
      ...
      baseTTC: parseFloat((listTtc * exchangeRate).toFixed(3))
    };
  }, [rows, exchangeRate]);

  // 11. Form Submission Validations
  const validateForm = () => {
    if (!selectedSupplier) {
    ...
    if (!selectedSite) {
      toast.error("Veuillez sélectionner un site de stockage de destination.");
      return false;
    }
    if (rows.length === 0) {
      toast.error("Le document doit contenir au moins une ligne d'article.");
      return false;
    }
    ...
    try {
      ...
      const merchandisesPayload = rows.map(r => {
        ...
        if (r.line_type === LineType.TransportFee) {
          ...
          item.article = null;
          item.packagereference = 'Standard';
        } else {
          item.article = {
            ...r.selectedArticle,
            lastpurchaseprice_ttc: r.unit_price_ht // map standard unit price selected to article card
          };
          item.lisoflengths = r.listLengths;
          item.packagereference = 'Standard'; // Standard package reference for orders
          item.isinvoicible = true;
          item.allownegativstock = false;
      ...
      // Invalidate queries cache to sync data lists immediately
      ...
      toast.error("La création de la commande a échoué. Veuillez vérifier les paramètres.");
    } finally {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-500 font-medium text-xs">
                Générez des bons de commande pour vos fournisseurs et gérez vos approvisionnements sans impact de stock immédiat.
              </p>
            </div>
          </div>
          ...
          <Card className="lg:col-span-2 border-slate-200/60 shadow-xl shadow-slate-900/5 rounded-[24px] overflow-hidden bg-white">
            ...
            <CardContent className="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="space-y-2">
                <label className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest block font-mono">Référence Interne / Note</label>
                <Input
                  className="h-11 rounded-xl border-slate-200 focus:ring-amber-900 bg-slate-50/50 text-xs font-bold text-slate-900"
                  placeholder="Ex: COF-2026-X"
              ...
              {/* Destination Site */}
              <div className="space-y-2">
                <label className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest block font-mono">Dépôt Associé *</label>
                <Select 
                  onValueChange={(val) => {
              ...
              {/* Status */}
              <div className="space-y-2">
                <label className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest block font-mono">Statut Commande *</label>
                <Select 
                  onValueChange={(val) => setDocStatus(parseInt(val || '9') as DocStatus)} 
                ...
                >
                  ...
                  <SelectContent className="rounded-xl border-slate-200 font-bold text-xs">
                    ...
                    <SelectItem value={DocStatus.Sent.toString()} className="font-bold text-xs">Envoyé</SelectItem>
                    <SelectItem value={DocStatus.PartiallyDelivered.toString()} className="font-bold text-xs">Partiellement livré</SelectItem>
                    <SelectItem value={DocStatus.Delivered.toString()} className="font-bold text-xs">Livré</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  type="number"
                  step="0.000001"
                  className="h-11 rounded-xl border-slate-200 focus:ring-amber-900 bg-slate-50/50 text-xs font-bold text-slate-900 font-mono"
                  value={exchangeRate}
              ...
              <div className="space-y-2 md:col-span-2">
                <label className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest block font-mono">Notes / Observations</label>
                <Input
                  className="h-11 rounded-xl border-slate-200 focus:ring-amber-900 bg-slate-50/50 text-xs font-bold text-slate-900"
                  placeholder="Informations supplémentaires ou remarques sur la commande..."
                  value={notes}
        ...
        <Card className="border-slate-200/60 shadow-xl shadow-slate-900/5 rounded-[24px] overflow-hidden bg-white">
          ...
          <CardContent className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
              ...
              {/* Last Purchase Price HT display (Read-Only) */}
              <div className="space-y-1.5 md:col-span-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block font-mono">Dernier Prix d&apos;Achat (HT)</label>
                <div className="relative">
                  <Input
                    disabled
                    type="number"
                    step="0.001"
                    className="h-10 rounded-xl border-slate-200 bg-slate-100 text-slate-500 text-xs font-bold font-mono pr-12"
                    value={isLoadingLastPrice ? '' : newRowLastPurchasePrice}
                  />
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-slate-400 font-mono">
                    {isLoadingLastPrice ? '...' : docCurrency}
                  </span>
                </div>
              </div>

              {/* Description/Supplier Ref per Line */}
              <div className="space-y-1.5 md:col-span-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block font-mono">Réf Fourn. Ligne / Note</label>
                <Input
                  className="h-10 rounded-xl border-slate-200 focus:ring-amber-900 text-xs font-bold text-slate-900 bg-slate-50/50"
                  placeholder="Note ou réf fournisseur ligne..."
            ...
            {/* Logistics Transporters Separator */}
            <div className="relative my-6">
              <div className="absolute inset-0 flex items-center" aria-hidden="true">
                <div className="w-full border-t border-slate-100"></div>
              </div>
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-white px-3 font-bold text-slate-400 tracking-widest font-mono">Logistique & Frais de Transport</span>
              </div>
            </div>

            {/* Transport Logistics Grid */}
                    ...
                    {rows.length === 0 ? (
                      <tr>
                        <td colSpan={9} className="p-12 text-center text-slate-400 font-medium italic">
                          Aucun article ou ligne logistique n&apos;a encore été inséré à la commande.
                        </td>
                      </tr>
                    ) : (
                      rows.map((row, idx) => {
                          ...
                          >
                            <td className="p-4 text-center font-bold text-slate-400 font-mono">
                              <div className="flex items-center justify-center gap-1">
                                <span>{idx + 1}</span>
                                {isTransport && <Truck className="w-3.5 h-3.5 text-amber-700 shrink-0" />}
                              </div>
                            </td>
                            <td className="p-4">
                              {isTransport ? (
                                <div className="text-amber-900 font-bold">
                                  ...
                                  <span className="text-[10px] text-slate-400 block font-normal">Services logistiques</span>
                                </div>
                              ) : (
                                <div>
                                  <span className="font-bold text-slate-900 block">{row.selectedArticle?.reference}</span>
                                  <span className="text-[10px] text-slate-500 block truncate max-w-[280px]">{row.selectedArticle?.description}</span>
                                </div>
                              )}
                            </td>
                            <td className="p-3">
                              {isTransport ? (
                                <div className="flex gap-2">
                                  <Input
                                    ...
                                    className="h-8 rounded-lg border-slate-200 focus:ring-amber-900 bg-slate-50/50 text-[11px] font-bold flex-2"
                                  />
                                  <Select 
                                    value={row.transporter_id?.toString() || ''}
                                    onValueChange={(val) => {
                                      const trans = allTransporters.find(t => t.id.toString() === val);
                                      if (trans) {
                                        setRows(prev => prev.map((r, i) => i === idx ? {
                                          ...
                                          transporter_name: trans.fullname,
                                          description: `Frais de transport - ${trans.fullname}`
                                        } : r));
                                      }
                                    }}
                                  >
                                    <SelectTrigger className="h-8 rounded-lg border-slate-200 focus:ring-amber-900 bg-white text-[11px] font-bold flex-1">
                                      <SelectValue />
                                    </SelectTrigger>
                                ...
                                <Input
                                  ...
                                  placeholder="Note fournisseur ligne"
                                />
                              )}
                            </td>
                            <td className="p-3">
                            ...
                            <td className="p-4 font-mono font-bold text-slate-500">
                              {isTransport ? '—' : (row.selectedArticle?.lastpurchaseprice_ttc ? `${row.selectedArticle.lastpurchaseprice_ttc.toFixed(3)}` : '—')}
                            </td>
    ...
    <Suspense fallback={
      <div className="flex items-center justify-center min-h-screen bg-slate-50/50">
        <div className="text-center space-y-4">
          ...
          <p className="text-xs text-slate-400 font-bold tracking-widest uppercase font-mono">Chargement du module de commande...</p>
        </div>
      </div>
    }>
      <NewSupplierOrderPageContent />
    </Suspense>
  );
}
