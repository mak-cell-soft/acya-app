...
function EditSupplierReceiptPageContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const queryClient = useQueryClient();

  // Guard: block direct URL access if user lacks canUpdate on purchases
  const { hasPermission } = usePermissionGuard();
  useEffect(() => {
    if (!hasPermission('purchases', 'canUpdate')) {
      toast.error("Vous n'avez pas la permission de modifier des bons de réception.");
  ...
  const receiptId = params.id ? parseInt(params.id as string) : 0;
  ...
  // 5. Load Existing Receipt for Editing
  useEffect(() => {
    if (receiptId <= 0 || !allArticles.length || !allSuppliers.length || !allSites.length) return;
    setIsLoading(true);

    documentService.getById(receiptId)
      .then((doc: any) => {
        if (doc.isinvoiced) {
          toast.error("Ce bon de réception est déjà facturé et ne peut plus être modifié.");
          router.replace('/purchases');
          return;
        }
        ...
        // Set storage site
        if (doc.sales_site) {
          const site = allSites.find(s => s.id === doc.sales_site.id);
          if (site) {
        ...
        if (doc.merchandises) {
          const mapped = doc.merchandises.map((m: any) => {
            ...
            const importQuantity = m.quantity || 0;
            ...
            const r: MerchandRow = {
              ...
              isGlassArticle: m.article ? m.article.unit?.toUpperCase() === 'M2' : false,
        ...
        toast.info(`Bon de réception Réf: ${doc.docnumber} chargé avec succès.`);
      })
      .catch(err => {
        console.error('Error fetching receipt:', err);
        toast.error('Impossible de charger le bon de réception.');
      })
      .finally(() => setIsLoading(false));
  }, [receiptId, allArticles, allSuppliers, allSites]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const documentPayload: any = {
        ...
        docnumber: '', // Retained by backend or not sent empty if not needed
        ...
        isdeleted: false,
        regulationid: 0,
        editing: true, // Mark as edit
        docstatus: DocStatus.Created, // Keep current status or fetch original
        isservice: false,
        isPaid: false,
      ...
      await documentService.update(receiptId, documentPayload);
      ...
      if (err.response?.status === 409) {
        toast.error("Une erreur de conflit est survenue lors de la mise à jour.");
      } else {
        toast.error("La modification du document a échoué. Veuillez vérifier les paramètres.");
      }
    } finally {
      setIsLoading(false);
    }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">
                Modifier Bon de Réception (BR)
              </h1>
              <p className="text-slate-500 font-medium text-xs">
                Modifiez les marchandises et les quantités de ce bon de réception.
              </p>
            </div>
          </div>
...
export default function EditSupplierReceiptPage() {
  return (
    <Suspense fallback={
      ...
      <EditSupplierReceiptPageContent />
    </Suspense>
  );
}
