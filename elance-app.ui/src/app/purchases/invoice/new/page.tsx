...
function NewSupplierInvoicePageContent() {
  ...
  // Stamp Taxes & RS Hooks
  const { data: appvariablesTaxes = [] } = useAppVariables('Taxe'); // Timbre
  const { data: appvariablesRS = [] } = useAppVariables('RS');     // RS
  ...
  const [supplierReference, setSupplierReference] = useState<string>(''); // Facture Réf (Required)
  ...
  useEffect(() => {
    if (appvariablesTaxes.length > 0) {
      const activeStampTaxes = appvariablesTaxes.filter(t => !t.isdeleted);
      const defaultTax = activeStampTaxes.find(t => t.isdefault === true) || activeStampTaxes[0];
  ...
  const handleGenerateReference = async () => {
    ...
    try {
      ...
      // Check our local front-end rows list to find the highest increment and increment sequentially
  ...
  const totals = useMemo(() => {
    ...
    const stampAmount = selectedTax ? parseFloat(selectedTax.value || '0') : 0;
    const rsPercent = selectedRS ? parseFloat(selectedRS.value?.replace('%', '') || '0') : 0;
    const rsTaxValue = listHTNet * (rsPercent / 100);

    const payable = listTtc + stampAmount - rsTaxValue;
    ...
    return {
      ...
      stampAmount: parseFloat(stampAmount.toFixed(3)),
      rsTaxValue: parseFloat(rsTaxValue.toFixed(3)),
      payable: parseFloat(payable.toFixed(3))
    };
  }, [rows, selectedTax, selectedRS]);
  ...
  const validateForm = () => {
    ...
    if (!supplierReference.trim()) {
      toast.error("Veuillez saisir la Référence Facture Fournisseur.");
      return false;
    }
    if (!selectedSite) {
    ...
    if (!selectedTax) {
      toast.error("Veuillez sélectionner un timbre fiscal.");
      return false;
    }
    if (rows.length === 0) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Type is 3 for Supplier Invoice (Facture Fournisseur - FA)
      // Stock transaction type is 1 (entrance of stock / store inventory, updates stock for direct invoice)
      const documentPayload: any = {
        ...
        regulationid: 0,
        editing: false,
        docstatus: DocStatus.Validated, // Validated
        isservice: false,
        isPaid: false,
      ...
      if (selectedRS) {
        documentPayload.holdingtax = {
          ...
          taxpercentage: parseFloat(selectedRS.value?.replace('%', '') || '0'),
          taxvalue: totals.rsTaxValue,
      ...
      toast.success(`Facture Fournisseur créée avec succès ! Réf: ${docRef}`);
      ...
      if (err.response?.status === 409) {
        toast.error("Une facture avec la même référence existe déjà.");
      } else {
        toast.error("La création de la facture a échoué. Veuillez vérifier les paramètres.");
      }
    } finally {
      setIsLoading(false);
    }
    ...
    <DashboardLayout>
      <div className="space-y-8 animate-in fade-in duration-700">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">
                Nouvelle Facture Fournisseur (FA)
              </h1>
              <p className="text-slate-500 font-medium text-xs">
                Enregistrez une facture directe de vos fournisseurs tout en alimentant vos dépôts de stockage.
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-2 border-slate-200/60 shadow-xl shadow-slate-900/5 rounded-xl overflow-hidden bg-white">
            <CardHeader className="bg-corp-blue-50/90 text-corp-blue-950 border-b border-corp-blue-100 p-5">
              ...
              <CardDescription className="text-xs text-slate-600 font-medium">
                Configurez le fournisseur, la référence facture et les paramètres financiers.
              </CardDescription>
            </CardHeader>
            <CardContent className="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              {/* Reference Facture (supplierReference) */}
              <div className="space-y-2">
                <label className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest block font-mono">Référence Facture Fournisseur *</label>
                <Input
                  className="h-11 rounded-xl border-slate-200 focus:ring-amber-900 bg-slate-50/50 text-xs font-bold text-slate-900"
                  placeholder="Saisir la référence de la facture..."
              ...
              {/* Date */}
              <div className="space-y-2">
                <label className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest block font-mono">Date Facture *</label>
                <div className="relative">
              ...
              {/* Stamp Tax (Timbre Fiscal) - Mandatory */}
              <div className="space-y-2">
                <label className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest block font-mono">
                  <Landmark className="inline w-3 h-3 mr-1" /> Timbre Fiscal *
                </label>
                <Select
                ...
                >
                  <SelectTrigger className="h-11 rounded-xl border-slate-200 focus:ring-amber-900 bg-slate-50/50 text-xs font-bold text-slate-900">
                    <SelectValue placeholder="Sélectionner le timbre fiscal...">
                      {selectedTax ? `${selectedTax.name} (${parseFloat(selectedTax.value || '0').toFixed(3)} TND)` : 'Choisir un timbre...'}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent className="rounded-xl border-slate-200 font-bold text-xs">
                    {appvariablesTaxes.filter(t => !t.isdeleted).map(tax => {
                      const label = `${tax.name} (${parseFloat(tax.value || '0').toFixed(3)} TND)`;
                      return (
              ...
              {/* Withholding Tax (Retenue à la source) - Optional */}
              <div className="space-y-2">
                <label className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest block font-mono">Retenue à la Source (RS)</label>
                <Select
                ...
                >
                  <SelectTrigger className="h-11 rounded-xl border-slate-200 focus:ring-amber-900 bg-slate-50/50 text-xs font-bold text-slate-900">
                    <SelectValue placeholder="Aucune Retenue à la source">
                      {selectedRS ? `${selectedRS.name} (${selectedRS.value})` : 'Aucune Retenue à la source'}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent className="rounded-xl border-slate-200 font-bold text-xs">
                    <SelectItem value="none" className="font-bold text-xs">Aucune Retenue à la source</SelectItem>
                    {appvariablesRS.filter(r => !r.isdeleted).map(rs => {
          ...
          <Card className="border-slate-200/60 shadow-xl shadow-slate-900/5 rounded-xl overflow-hidden bg-corp-blue-50/90 text-corp-blue-950 border-b border-corp-blue-100">
            <CardHeader className="border-b border-corp-blue-100 p-5 bg-corp-blue-50">
              <CardTitle className="text-sm font-bold text-corp-blue-900 flex items-center gap-2">
                <Coins className="w-4 h-4 text-corp-blue-500" /> Synthèse Financière (FA)
              </CardTitle>
            ...
            <CardContent className="p-6 space-y-4 font-mono text-xs font-bold">
              ...
              {totals.stampAmount > 0 && (
                <div className="flex justify-between items-center text-amber-800">
                  ...
                  <span className="text-sm">+{totals.stampAmount.toFixed(3)} {docCurrency}</span>
                </div>
              )}
              {totals.rsTaxValue > 0 && (
                <div className="flex justify-between items-center text-rose-700">
                  <span>RETENUE À LA SOURCE ({selectedRS?.value})</span>
                  <span className="text-sm">-{totals.rsTaxValue.toFixed(3)} {docCurrency}</span>
                </div>
              )}
              <div className="h-px bg-corp-blue-200 my-1" />
              <div className="flex justify-between items-center text-corp-blue-950 font-extrabold">
                <span>NET TTC (+ Timbre)</span>
                <span className="text-base text-corp-blue-950">{(totals.ttc + totals.stampAmount).toFixed(3)} {docCurrency}</span>
              </div>

              <div className="h-px bg-slate-800 my-1" />
              <div className="flex justify-between items-center text-amber-400 text-sm font-extrabold pt-2">
              ...
              <div className="pt-4 flex flex-col gap-2.5">
                ...
                >
                  {isLoading ? (
                    ...
                    <>
                      <CheckCircle2 className="w-4 h-4" /> Enregistrer la Facture
                    </>
                  )}
                </Button>
                <Button
        ...
        <Card className="border-slate-200/60 shadow-xl shadow-slate-900/5 rounded-xl overflow-hidden bg-white">
          <CardHeader className="bg-corp-blue-50/90 text-corp-blue-950 border-b border-corp-blue-100 p-5">
            <CardTitle className="text-sm font-bold text-corp-blue-900 flex items-center gap-2">
              <FileText className="w-4 h-4 text-corp-blue-500" /> Nomenclature des Articles Facturés
            </CardTitle>
            <CardDescription className="text-xs text-slate-600 font-medium">Liste des articles facturés. Modifiez les prix unitaires, remises et colis en ligne.</CardDescription>
          </CardHeader>
                    ...
                    {rows.length === 0 ? (
                      <tr>
                        <td colSpan={9} className="p-12 text-center text-slate-400 font-medium italic">
                          Aucun article ou ligne de transport n&apos;a encore été ajouté à la facture.
                        </td>
                      </tr>
                    ) : (
                      rows.map((row, idx) => {
