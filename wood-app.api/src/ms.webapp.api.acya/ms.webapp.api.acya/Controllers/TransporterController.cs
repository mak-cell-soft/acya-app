...
namespace ms.webapp.api.acya.api.Controllers
{
  public class TransporterController : BaseApiController
  {
    ...
    [HttpPut("{id}")]
    public async Task<ActionResult<TransporterDto>> Update(int id, TransporterDto dto)
    {
      if (dto == null) return BadRequest("Transporter data is required");
      
      var transporter = await _repository.Get(id);
      ...
      if (!string.IsNullOrEmpty(dto.firstname) && !string.IsNullOrEmpty(dto.lastname))
      {
        string _fullname = Helpers.CapitalizeFirstLetter(dto.firstname) + " " + dto.lastname.ToUpper();
        var existingTransporter = await _repository.ExistsByFullName(_fullname);
        if (existingTransporter && transporter.FullName != _fullname)
        {
          return Conflict("Transporter with this name already exists");
        }
      }
      ...
      if (dto.car != null)
      {
        if (transporter.VehicleId.HasValue && transporter.VehicleId.Value > 0)
        {
          var vehicle = await _vehicleRepository.Get(transporter.VehicleId.Value);
          if (vehicle != null)
          {
            var origVehicleId = vehicle.Id;
            vehicle.UpdateFromDto(dto.car);
            vehicle.Id = origVehicleId; // Prevent PK modification
            await _vehicleRepository.Update(vehicle);
          }
        }
        else
        {
          var vehicle = new Vehicle(dto.car);
          // New vehicle id is generated by db
          vehicle = await _vehicleRepository.Add(vehicle);
          transporter.VehicleId = vehicle.Id;
        }
      }

      var origTransporterId = transporter.Id;
      transporter.UpdateFromDto(dto);
      transporter.Id = origTransporterId; // Prevent PK modification
      
      var updatedTransporter = await _repository.Update(transporter);

      if (updatedTransporter == null)
      {
        return BadRequest("Failed to update the transporter");
      }

      return Ok(new TransporterDto(updatedTransporter));
    }

    [HttpDelete("{id}")]
    public async Task<ActionResult> Delete(int id)
    {
      var deletedTransporter = await _repository.Delete(id);
      if (deletedTransporter == null)
      {
        return BadRequest("Failed to delete the transporter or transporter not found");
      }
      return Ok();
    }
  }
